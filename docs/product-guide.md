...
- **Wake word:** "Hey Elio"
- **Commands:**
  - *"Hey Elio, next"* — advance to next step
  - *"Hey Elio, back"* — go to previous step
  - *"Hey Elio, repeat"* — reads current step aloud again
  - *"Hey Elio, done"* — turns off voice control (stays in hands-free mode; use the exit button to leave)
- **Text-to-speech** reads each step aloud — starts automatically after dismissing the voice help dialog
- **Continuous listening** with auto-restart on silence
- Toggle via the microphone button in hands-free mode
- Requires microphone permission (Android prompts on first use)
...
Bring external recipes into your Recipe Book — useful for saving recipes from books, websites, or cards.

**Two import methods:**

- **Photo scan** — Take a photo of a recipe (book page, handwritten card, website screenshot). Gemini Vision extracts the title, description, ingredients, and steps automatically. Review and save.
- **Manual entry** — Type a recipe in directly. Dynamic ingredient rows (quantity + name), free-text instructions, auto-parsed into structured steps.

Imported recipes are saved to your Recipe Book immediately and appear alongside generated recipes. Entry point: **Profile → Recipe Book → Import recipe** button (Pro users only).

### 9. Bulk Prep Mode (Pro)
