            ...
            handler.post {
                run {
                    recognizerIntent = Intent(RecognizerIntent.ACTION_RECOGNIZE_SPEECH).apply {
                        ...
                        // Elio patch (20 May 2026) — extend the silence-
                        // length EXTRAs that the upstream plugin never
                        // set. Android's `SpeechRecognizer` defaults to
                        // ~1.5–4s of silence before ending a session
                        // (device-dependent — Rob's device showed 4s in
                        // 20may-d testing), which created a constant
                        // restart cycle in Cook Mode and dead-zones where
                        // mid-restart speech got dropped. Bumping both
                        // "possibly complete" and "complete" silence to
                        // 15s holds the session open ~4× longer, slashing
                        // restart cadence. `MINIMUM_LENGTH_MILLIS: 0` lets
                        // the engine respond immediately to short
                        // commands ("next") without a minimum capture.
